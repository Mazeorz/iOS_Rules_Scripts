...
function setConsumerProtection(data, consumerProtection) {
    ...
    if (data.ok == 1 && data.single) {
        ...
        const item = customItem(lower[1], [`${lower[0]}\n【提示】${tip}（仅供参考）\n${summary}`])
...
function lowerMsgs(data) {
    ...
    const lowerMsg = "【最低价格】 ¥" + String(lower) + `（${lowerDate}）`
    const lowerMsg1 = "历史价格" //+ String(lower)
...
function priceSummary(data) {
    ...
    list.forEach((item, index) => {
        if (item.Name == "双11价格") {
            ...
            item.Name = "三十天最低"
        } else if (item.Name == "30天平均价") {
        return
        } else if (item.Name == "180天最低价") {
        return
        }
        summary += `\n-${item.Name}:${getSpace(2)}${item.Price.padEnd(10)}${item.Date}${getSpace(4)}${item.Difference}`
        //summary += `\n${item.Name}${getSpace(4)}${item.Price}${getSpace(4)}${item.Date}${getSpace(4)}${item.Difference}`
        let summaryItem = `${item.Name}:${getSpace(2)}${item.Price.padEnd(10)}${item.Date}${getSpace(4)}${item.Difference}`
